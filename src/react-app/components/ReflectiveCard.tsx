  ...
  return (
    <div className={`reflective-card-container ${className}`} style={{ ...style, ...cssVariables }}>
      <svg className="reflective-svg-filters" aria-hidden="true">
        <defs>
          <filter id={`metallic-displacement-${id}`} x="-20%" y="-20%" width="140%" height="140%">
      ...
      {useImage && image ? (
        <img
          ...
          style={{ filter: `saturate(${saturation}) contrast(120%) brightness(110%) blur(${blurStrength}px) url(#metallic-displacement-${id})` }}
        />
      ) : (
