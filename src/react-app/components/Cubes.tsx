...
const Cubes: React.FC<CubesProps> = ({
  ...
  useEffect(() => {
    const targetEl = (() => {
      ...
      // Prefer the parent element's parent (e.g., section) so the target
      // receives pointer events even when cubes are rendered behind content.
      const parent = sceneRef.current.parentElement;
      const grand = parent?.parentElement;
      return (grand as HTMLElement) || (parent as HTMLElement) || sceneRef.current;
    })();
    if (!targetEl) return;
